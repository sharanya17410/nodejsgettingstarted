...
app.post('/test/upload', (req, res) => {
  ...
  // Use the mv() method to place the file somewhere on your server
  sampleFile.mv(path.join(__dirname, '/public/img/Reunion/') + sampleFile.name, function (err) {
    if (err)
      return res.status(500).send(err);

    //joining path of directory 
    const directoryPath = path.join(__dirname, '/public/img/Reunion/');
    //passsing directoryPath and callback function
    fs.readdir(directoryPath, function (err, files) {
      //handling error
      if (err) {
      ...
      files.forEach(function (file) {
        ...
        request.post({
          headers: { 'Authorization': 'Client-ID ' + 'c2049e40de14fa8' },
          url: 'https://api.imgur.com/3/image',
          formData: { image: fs.createReadStream(path.join(__dirname, '/public/img/Reunion/'+file)) },
        }, function (err, response, body) {
          if (err) {
            console.log(err);
          }
